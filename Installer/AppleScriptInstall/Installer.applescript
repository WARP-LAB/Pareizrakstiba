...
if dialog1Answer is "Install" then
	do shell script "/bin/bash " & thisPath & "Installer.app/Contents/Resources/payload/assets/uninstall.sh" with administrator privileges
	do shell script "/bin/bash " & thisPath & "Installer.app/Contents/Resources/payload/assets/enable-unsigned.sh" with administrator privileges
	do shell script "/bin/bash " & thisPath & "Installer.app/Contents/Resources/payload/assets/install.sh"
	if button returned of (display dialog "Do you wish to set Latvian as preferred spelling language everywhere?" buttons {"No", "Yes"} default button 2 with title "Pareizrakstiba") is "Yes" then
		do shell script "/bin/bash " & thisPath & "Installer.app/Contents/Resources/payload/assets/force-default.sh" with administrator privileges
	end if
	do shell script "/bin/bash " & thisPath & "Installer.app/Contents/Resources/payload/assets/firstopen.sh"
